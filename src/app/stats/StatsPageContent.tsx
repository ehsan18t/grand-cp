...
export function StatsPageContent() {
  ...
  const totalProblems = useAppStore((s) => s.totalProblems);
  const phaseCountsMap = useAppStore((s) => s.phaseCountsMap);
  const statusCounts = useAppStore((s) => s.statusCounts);
  const phaseSolvedMap = useAppStore((s) => s.phaseSolvedMap);
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      {/* Guest CTA Banner */}
      {isGuest && (
        <div className="mb-8 overflow-hidden rounded-xl border border-primary/20 bg-primary/5 p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="mb-1 font-semibold text-lg">Ready to track your progress?</h2>
